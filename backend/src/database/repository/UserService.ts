...
export class UserDetailsService extends Database {
  constructor() {
    super();
    this.CreateTable(
      'create  table if not exists UserDetails ( userID int primary key AUTO_INCREMENT, email varchar(100) unique ,encrptedPassword varchar(100),mob varchar(100),  fname varchar(100),lname varchar(100),dob varchar(100), gender varchar(100),role int ,status int) '
    );
    ...
    this.CreateTable('create  table if not exists contactDetails ( ID int primary key AUTO_INCREMENT, email varchar(100)  ,  fname varchar(100),lname varchar(100), message text) ');
  }
  ...
  public getAuthUser = async (res: express.Response, email: string, password: string) => {
    ...
    this.executeQuery(
      `select userID ,fname , lname,email , mob ,gender , role , status , dob from userdetails  where email = '${email}' and encrptedPassword = '${encrptedPassword}' `,
      (err: any, data: any) => {
        const result = {
          status: '',
          error: '',
  ...
  public insertAuthUser = async (res: express.Response, email: string, password: string, mob: string, role: string, status: string, lname: string, fname: string, dob: string, gender: string) => {
    ...
    const encrptedPassword = '' + crypto.SHA256(password);
    this.executeQuery(
      `insert into userDetails values ( default , '${email}', '${encrptedPassword}', '${mob}', '${fname}', '${lname}', '${dob}', '${gender}', ${role}, ${status} ) ;`,
      (err: any, data: any) => {
        const result = {
          status: '',
          error: '',
  ...
  public updateUser = async (res: express.Response, lname: string, fname: string, dob: string, gender: string, email: string, userID: number) => {
    this.setMySqlConfig('complain');
    // console.log(address, pincode, LandMarks, userID, ComplainDetails)
    this.executeQuery(`update  userdetails set fname="${fname}", lname="${lname}", email="${email}" , dob="${dob}", gender="${gender}" where userID=${userID};`, (e: any, pm: any) => {
      const result = {
        status: '',
        error: '',
        token: '',
  ...
  public updateContactUs = async (res: express.Response, lname: string, fname: string, email: string, message: string) => {
    this.setMySqlConfig('complain');

    this.executeQuery(`insert into contactDetails values ( default ,"${email}", "${fname}","${lname}","${message}") ;`, (e: any, response: any) => {
      const result = {
        status: '',
        error: '',
        message: '',
      };
      this.executeQuery(`select ID from contactDetails where email = '${email}';`, (err: any, select: any) => {
        result['status'] = 'success';
