...
export class UserDetailsController extends BaseController implements Controller {
  ...
  private _initialiseRoutes = () => {
    ...
    this.router.post(`${this.path}/contactus`, this._contactus);
  ...
  private _contactus = this.catchAsyn(async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    const { lname, fname, email, message } = req.body;

    console.log(req.url);

    this.getUserDetails.updateContactUs(res, lname, fname, email, message);
  });
}
